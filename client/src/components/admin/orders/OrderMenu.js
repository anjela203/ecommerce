...
const OrderMenu = (props) => {
  ...
  const [filteredTransactionId, setFilteredTransactionId] = useState("");
  ...
  useEffect(()=>{
    if(filteredTransactionId){
      ...
      filterOrder("resetFilter", data, dispatch, dropdown, setDropdown)
    }
  },[filteredTransactionId])
  return (
    <Fragment>
      <div className="col-span-1 flex items-center">
        <div className="flex flex-col space-y-4 md:flex-row md:justify-between md:items-center md:space-y-0 w-full">
          ...
          <div>
            <SearchFilter setFilteredTransactionId={setFilteredTransactionId}/>
          </div>
        </div>
        {/*<AddCategoryModal/>*/}
        <UpdateOrderModal />
